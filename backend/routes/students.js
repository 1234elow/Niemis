...
const normalizeGradeFilter = (rawValue) => {
...
const normalizeActiveFilter = (isActiveValue, statusValue) => {
  ...
  if (isActiveValue == null || isActiveValue === "") {
    return true;
  }
  ...
  const normalized = String(isActiveValue).trim().toLowerCase();
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      // Build where clause
      const whereClause = {
        is_active: normalizeActiveFilter(is_active, status),
      };
      if (school_id) whereClause.school_id = school_id;
      if (grade_level) {
        const normalizedGradeLevel = normalizeGradeFilter(grade_level);
        whereClause[Op.and] = whereClause[Op.and] || [];
        whereClause[Op.and].push(
          sequelize.where(
            sequelize.fn("LOWER", sequelize.col("Student.grade_level")),
            String(normalizedGradeLevel).toLowerCase(),
          ),
        );
      }
      if (search) {
        ...
        whereClause[Op.and] = whereClause[Op.and] || [];
        whereClause[Op.and].push({ [Op.or]: searchFilter });
      }

      // Enforce school scope at query level for all school-scoped roles.
      if (req.accessContext?.scope === "school") {
        let scopedSchoolId = req.accessContext?.school_id || null;
        if (!scopedSchoolId && req.user.role === "teacher") {
          const teacherStaff = await findTeacherStaffByUser(req.user);
          scopedSchoolId = teacherStaff?.school_id || null;
        }

        if (!scopedSchoolId) {
          return res.status(403).json({
            error:
              "Access denied: your account is not linked to a school profile",
          });
        }

        whereClause.school_id = scopedSchoolId;
      }
...
router.get("/:id", async (req, res, next) => {
  try {
    ...
    if (req.user.role === "parent") {
      const parentAccess = student.Parents?.some(
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (createData.rfid_tag === "") {
        createData.rfid_tag = null;
      }

      const student = await Student.create(createData);

      logger.info(
        `New student created: ${student.first_name} ${student.last_name} by user ${req.user.id}`,
      );
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      if (updateData.rfid_tag === "") {
        updateData.rfid_tag = null;
      }

      await student.update(updateData);

      logger.info(
        `Student updated: ${student.first_name} ${student.last_name} by user ${req.user.id}`,
      );
...
router.patch("/:id/health", async (req, res, next) => {
  try {
    ...
    let healthRecord = await StudentHealth.findOne({
      where: { student_id: id },
    });
...
router.patch("/:id/family", async (req, res, next) => {
  try {
    ...
    let familyRecord = await FamilySocialAssessment.findOne({
      where: { student_id: id },
    });
...
// Set year-end status for individual student
router.put(
  "/:id/year-end-status",
  requireRole(["super_admin", "admin", "teacher"]),
  [
    body("status")
      .isIn(["promoted", "stop_down", "graduated"])
      .withMessage("Status must be promoted, stop_down, or graduated"),
  ...
  async (req, res, next) => {
    try {
      ...
      if (req.user.role === "teacher") {
        ...
        if (teacherStaff && teacherStaff.school_id !== student.school_id) {
          return res
            .status(403)
            .json({ error: "Access denied: Student is not in your school" });
        }
      }

      // Store old status for audit
      const oldStatus = student.year_end_status;

      await student.update({
        year_end_status: status,
        year_end_status_date: new Date(),
        year_end_status_set_by: req.user.id,
        year_end_notes: notes || null,
      });
      ...
      try {
        await AuditLog.create({
          user_id: req.user.id,
          action: "year_end_status_update",
          table_name: "students",
          record_id: student.id,
          old_values: { year_end_status: oldStatus },
          new_values: { year_end_status: status, notes },
          ip_address: req.ip,
          user_agent: req.get("User-Agent"),
        });
      } catch (auditError) {
      ...
      logger.info(
        `Year-end status set for student ${student.student_id}: ${status} by user ${req.user.id}`,
      );

      res.json({
        message: "Year-end status updated successfully",
        student: {
          id: student.id,
          ...
          year_end_status_date: student.year_end_status_date,
          year_end_notes: student.year_end_notes,
        },
      });
    } catch (error) {
      next(error);
